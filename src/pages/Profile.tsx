...
export default function Profile() {
  ...
  // ─── AI Avatar picker state ────────────────────────────────────────────────
  const [showAvatarPicker, setShowAvatarPicker] = useState(false);

  const handleAiAvatarSelect = (url: string) => {
    setForm((prev) => ({ ...prev, avatar_url: url }));
    setShowAvatarPicker(false);
    push({ message: 'AI avatar selected! Save your profile to apply it.', type: 'success' });
  };

  const AI_AVATARS_MALE = Array.from({ length: 20 }, (_, i) => ({
    url: `https://i.pravatar.cc/300?img=${i + 1}`,
    label: `Male Avatar ${i + 1}`,
  }));

  const AI_AVATARS_FEMALE = Array.from({ length: 20 }, (_, i) => ({
    url: `https://i.pravatar.cc/300?img=${i + 21}`,
    label: `Female Avatar ${i + 1}`,
  }));
    ...
    <div className="relative min-h-screen overflow-hidden bg-[radial-gradient(circle_at_top_left,_rgba(59,130,246,0.18),_transparent_35%),linear-gradient(135deg,_#f8fbff_0%,_#eef5ff_45%,_#f8fbff_100%)] text-slate-900">
      ...
      {showAvatarPicker && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-950/40 backdrop-blur-sm p-4">
          <div className="w-full max-w-3xl max-h-[90vh] overflow-y-auto rounded-[2rem] border border-slate-200/80 bg-white p-6 shadow-2xl">
            <div className="flex items-center justify-between gap-3 mb-6">
              <div>
                <h3 className="text-lg font-bold text-slate-900">Choose an AI Avatar</h3>
                <p className="text-sm text-slate-500 mt-1">Click an avatar to apply it, or upload your own photo.</p>
              </div>
              <button
                type="button"
            ...
            {/* Upload option */}
            <div className="mb-6 rounded-2xl border-2 border-dashed border-blue-200 bg-blue-50 p-4">
              <button
                type="button"
                onClick={() => { setShowAvatarPicker(false); setTimeout(() => avatarInputRef.current?.click(), 300); }}
                className="w-full flex items-center justify-center gap-2 text-blue-700 font-semibold text-sm"
              >
                <Camera className="h-5 w-5" />
            ...
            <div className="mb-6">
              ...
              <div className="grid grid-cols-4 sm:grid-cols-5 md:grid-cols-7 gap-2.5">
                {AI_AVATARS_MALE.map((avatar) => (
                  <button
                    key={avatar.label}
                    type="button"
                    onClick={() => handleAiAvatarSelect(avatar.url)}
                    className={`group relative rounded-xl overflow-hidden border-2 transition-all ${
                      form.avatar_url === avatar.url
            ...
            <div className="mt-6 flex justify-end">
              <button
                type="button"
                onClick={() => setShowAvatarPicker(false)}
                className="rounded-2xl bg-blue-600 px-6 py-2.5 text-sm font-semibold text-white hover:bg-blue-700 transition"
              >
                Done
              </button>
            </div>
