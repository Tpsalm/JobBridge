...
7. Include keywords from the job description`;
  ...
  const userPrompt = `
Tailor this resume for the "${jobTitle}" position:
...
- Highlights most relevant experience for this role
- Incorporates key skills and keywords from the job description
- Maintains professional formatting
- Shows measurable achievements
- Is ATS-friendly`;
...
6. Maintain professional tone`;
  ...
  const userPrompt = `
Create a cover letter for applying to:
- Position: ${jobTitle}
- Company: ${companyName}
  ...
  try {
    switch (body.type) {
      ...
      case 'cover-letter': {
        ...
        const companyName = (body as any).companyName || 'the company';
...
serve(async (req) => {
  if (handleCors(req)) return new Response(null, { headers: getCorsHeaders() });
