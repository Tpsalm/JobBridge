...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    // EVERY service provider appears in the public feed regardless of billing
    // state. Monetization is delivered through RANKING + BADGING instead of
    // hiding listings: providers who subscribed to the Featured Professional
    // package (is_featured) rank first, Verified Professional (is_verified)
    // second, and everyone else last. No visibility_until / is_active gate is
    // applied so no provider can silently vanish from the marketplace.
